...
def ask_question():
    ...
    current_session_name = request.form.get(
        "session_name", None
    )  # Get the current session name from the form
    ...
    if not current_session_name:
        ...
        session_numbers = [
            int(s.split("_")[-1]) for s in sessions if s.startswith("session_")
        ]
...
def index(session_name):
    ...
    user_response = load_jsonl(
        os.path.join(current_session_path, "users_response.jsonl")
    )
...
def display_profile(session_name, profile_id):
    ...
    picture_file = f"profilepicture_{profile_id}.png"
