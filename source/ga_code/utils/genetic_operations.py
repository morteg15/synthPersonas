...
def update_fitness_group(pop, toolbox):
...
def mutation_group(offspring, mutation_probability, toolbox):
    ...
    # Mutation
    for group in mutated_offspring:
        if random.random() < mutation_probability:
            # Choose a random index within the group to replace
            mutation_index = random.randint(0, len(group) - 1)
            
            # Replace the chosen persona within the group using the toolbox
            random_persona = toolbox.persona_attributes()
            group[mutation_index] = random_persona
            counter += 1
