from generate_mock_data.norwegian_lists import jobs,income_brackets,exercise_habits,marital_statuses, educations, influencers, movies, books, intresser, norwegianCounties, norwegianLivingConditions, boy_names, girl_names, last_names
import random
import os
import toml
...
def generate_typical_norman(gender=None, age_range=(18, 70)):
    if not gender:
...
def create_random_personas(session_path, pop_number=200, gender=None, age_range=(7, 18), ):

    for i in range(pop_number):
        random_person = generate_typical_norman(gender, age_range=age_range)
